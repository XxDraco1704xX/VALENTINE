...
function Rise() {
    ...
    let textOpacity = 0;
    const textFadeStart = 10; // delay before text starts fading
    const textFadeDuration = 70; // frames to fade in text
    ...
    timer2 = setInterval(() => {
        ...
        if (distance >= textFadeStart) {
            if (distance < textFadeStart + textFadeDuration) {
                textOpacity = (distance - textFadeStart) / textFadeDuration;
...
function playAudio(){
    ...
    playBtn.addEventListener('click', function() {
        audio.play();
        playBtn.style.display = 'none'; // Oculta el botón después del click
    });
}
